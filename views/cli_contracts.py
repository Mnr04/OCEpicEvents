...
from controllers.contracts import (
    create_contract, get_all_contracts, update_contract,
...
@click.option('--client-id', prompt='ID du Client', type=int, help='ID du client lié au contrat.')
@click.option('--total', prompt='Montant Total (€)', type=float, help='Montant total du contrat.')
@click.option('--reste', prompt='Reste à payer (€)', type=float, help='Montant restant à payer.')
@click.pass_context
def create(ctx, client_id, total, reste):
    ...
    if create_contract(client_id, total, reste, user.get('role')):
        ...
        click.secho(" Erreur : Client introuvable ou échec de création.", fg="red")

@contract_commands.command(name="list")
@click.option('--filtre', type=click.Choice(['tous', 'non-signes', 'non-payes']), default='tous', help="Filtrer la liste des contrats.")
@click.pass_context
...
@click.option('--montant', type=float, help='Nouveau montant total (Gestion uniquement).', default=None)
@click.option('--signe', is_flag=True, help='Passer le statut du contrat à SIGNÉ.')
@click.pass_context
def update(ctx, contract_id, montant, signe):
    ...
    if update_contract(contract_id, user.get('role'), user.get('user_id'), nouveau_statut=nouveau_statut, nouveau_montant=montant):
        click.secho(" Contrat mis à jour avec succès !", fg="green")
    else:
        click.secho(" Erreur : Contrat introuvable, ou vous n'avez pas les droits sur ce contrat.", fg="red")
...
def delete(ctx, contract_id):
    user = ctx.obj
    if not user or user.get('role') != 'Gestion':
        click.secho(" Accès refusé. Seule l'équipe Gestion peut supprimer.", fg="red")
        return

    if click.confirm(f"Êtes-vous sûr de vouloir supprimer le contrat {contract_id} ?"):
